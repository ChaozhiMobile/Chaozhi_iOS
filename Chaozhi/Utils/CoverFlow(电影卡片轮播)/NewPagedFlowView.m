...
////////////////////////////////////////////////////////////////////////////////////////////////////
//更新图片高度
-(void)setImgProportion:(CGFloat)imgProportion{
    _imgProportion = imgProportion;
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.frame.size.width * _imgProportion);
}
-(void)setHiddenPageControll:(BOOL)hiddenPageControll{
    _hiddenPageControll = hiddenPageControll;
    if (_hiddenPageControll) {
        if (!_pageControl) {
            _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 32, self.frame.size.width, 8)];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        NSInteger tag = pageIndex % self.orginPageCount;
        PGIndexBannerSubiew *bannerView = [self dequeueReusableCell];
        if (!bannerView) {
            bannerView = [[PGIndexBannerSubiew alloc] init];
            bannerView.tag = tag;
            bannerView.layer.cornerRadius = self.cornerRadius;
            bannerView.layer.masksToBounds = YES;
        }
        //在这里下载网络图片
        if (self.urlImageDataSource) {
            [bannerView.mainImageView sd_setImageWithURL:[NSURL URLWithString:self.urlImageDataSource[tag]] placeholderImage:[UIImage imageNamed:@""]];
        }
        if (self.imageDataSource) {
            bannerView.mainImageView.image = self.imageDataSource[tag];
        }
        bannerView.mainImageView.contentMode = self.contentMode;
        cell = bannerView;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    {
        [self initialize];
        self.frame = CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, frame.size.width * self.imgProportion);
    }
    return self;
}
...
- (void)reloadData {
    ...
    if (_needsReload) {
        ...
        if (self.isCarousel) {
            ...
            _pageCount = self.orginPageCount == 1 ? 1: self.orginPageCount;
        }
        ...
        //重置pageWidth
        _pageSize = CGSizeMake(self.bounds.size.width - 4 * self.leftRightMargin,(self.bounds.size.width - 4 * self.leftRightMargin) * self.imgProportion);
